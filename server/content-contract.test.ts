...
describe('content API contract', () => {
  ...
  it('serves section and lesson detail endpoints using shared content schemas', async () => {
    ...
    try {
      ...
      expect(sectionDetailsSchema.safeParse(planningSectionResponse.json()).success).toBe(true)
      ...
      expect(digitalFootprintLessonResponse.json().next.lesson.slug).toBe('why-reserve-matters')
      expect(whyReserveLessonResponse.statusCode).toBe(200)
      expect(lessonDetailsSchema.safeParse(whyReserveLessonResponse.json()).success).toBe(true)
      expect(whyReserveLessonResponse.json().previous.lesson.slug).toBe('digital-footprint-and-protection')
      expect(whyReserveLessonResponse.json().next.lesson.slug).toBe('reserve-target-amount')
      expect(reserveTargetLessonResponse.statusCode).toBe(200)
      expect(lessonDetailsSchema.safeParse(reserveTargetLessonResponse.json()).success).toBe(true)
      expect(reserveTargetLessonResponse.json().previous.lesson.slug).toBe('why-reserve-matters')
      expect(reserveTargetLessonResponse.json().next.lesson.slug).toBe('pay-yourself-first')
      expect(payYourselfLessonResponse.statusCode).toBe(200)
      expect(lessonDetailsSchema.safeParse(payYourselfLessonResponse.json()).success).toBe(true)
      expect(payYourselfLessonResponse.json().previous.lesson.slug).toBe('reserve-target-amount')
      expect(payYourselfLessonResponse.json().next.lesson.slug).toBe('budget-draft')
