...
### 9.3.1. Passive Text Blocks

Passive educational cards such as `theory`, `callout`, `summary`, read-only
`scenario`, and read-only `single_choice` render author text as ordinary
paragraphs for now. Do not infer special surfaces from plain-text prefixes such
as `Факт`, `Формула`, `Пример`, or `Простой тест`.

Structure:
1. Card frame title as the only main heading.
2. Short paragraphs in `body.lg` / `text.secondary`.
3. Facts, formulas, examples, and simple tests stay inside the relevant
   Markdown-enabled text field as normal prose until a dedicated rich-text
   renderer introduces explicit supported components.
4. Examples may continue as quiet rounded chips when the card uses the
   `examples` array.

Do not use this pattern for interactive answer rows, reflection inputs,
artifact fields, or primary feedback states. Those keep their own
selectable/input/feedback components.
