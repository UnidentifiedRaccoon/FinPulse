...
DEFAULT_OPERATION_WAIT_SECONDS = 540
DEFAULT_LABEL_WAIT_SECONDS = 45
DEFAULT_OPERATION_POLL_SECONDS = 5
TRANSIENT_STATUSES = {"STARTING", "UPDATING", "STOPPING"}
...
def start(event, context):
    ...
    try:
        if status == "STOPPED":
            ...
            start_result = _wait_operation(config, start_operation["id"])
            if not start_result.get("done"):
                return _response(
                    202,
                    {
                        **base_body,
                        ...
                        "start_operation_done": False,
                        "message": "Database start is still running. Retry the start URL in a few minutes to write the lease label.",
                    },
                )

            refreshed_cluster = _get_cluster(config)
            refreshed_status = refreshed_cluster.get("status")
            if refreshed_status != "RUNNING":
                return _response(
                    202,
                    {
                        **base_body,
                        "action": "start-completed-not-ready",
                        "status": refreshed_status,
                        "label_operation_id": None,
                        "label_operation_done": False,
                        "start_operation_id": start_operation["id"],
                        "start_operation_done": True,
                        "message": "Database start operation completed, but the cluster is not RUNNING yet. Retry the start URL in a few minutes.",
                    },
                )

            label_operation = _update_labels(config, _merged_labels(refreshed_cluster, active_until))
                ...
                return _response(
                    202,
                    {
                        **base_body,
                        ...
                        "start_operation_id": start_operation["id"],
                        "start_operation_done": True,
                        "message": "Database started, but lease label update is still running. Retry the start URL in a minute.",
                    },
                )
            ...
            return _response(
                200,
                {
                    **base_body,
                    ...
                    "start_operation_id": start_operation["id"],
                    "start_operation_done": True,
                    "message": "Database start completed and the lease label was updated. Backend readiness may still take a few minutes.",
                },
            )
            ...
            return _response(
                202,
                {
                    **base_body,
                    ...
                    "label_operation_done": False,
                    "start_operation_id": None,
                    "message": "Cluster operation is already in progress. Check backend readiness in a few minutes.",
                },
            )
...
def _load_config():
    cluster_id = os.environ.get("FINPULSE_DB_CLUSTER_ID")
    if not cluster_id:
        raise RuntimeError("FINPULSE_DB_CLUSTER_ID is required")

    session_hours = _parse_int(os.environ.get("FINPULSE_DB_SESSION_HOURS"))
    if session_hours is None:
        session_hours = DEFAULT_SESSION_HOURS

    wait_seconds = _parse_int(os.environ.get("FINPULSE_DB_OPERATION_WAIT_SECONDS"))
    if wait_seconds is None:
        wait_seconds = DEFAULT_OPERATION_WAIT_SECONDS

    label_wait_seconds = _parse_int(os.environ.get("FINPULSE_DB_LABEL_WAIT_SECONDS"))
    ...
    while time.time() < deadline:
        ...
        if last_payload.get("done"):
            if "error" in last_payload:
                raise OperationError(json.dumps(last_payload["error"], ensure_ascii=False))
            return last_payload
...
def _merged_labels(cluster, active_until):
    ...
    labels["managed_by"] = "finpulse-db-control"
    return labels
